...
All tokens live in `designs/tokens.jsx` (the `TOKENS_CSS` constant). Port them verbatim to `src/app/globals.css`. They use OKLCH for a unified palette across **eight named themes**; each theme is exposed via a class on the root element (or `<body>`).

### 5.0 Theme system

The app supports a runtime theme switcher. All eight themes share the same token names — only the values change — so components reference tokens like `var(--accent)` and never care which theme is active.

| Theme key | Class | Dark? | Vibe |
|---|---|---|---|
| `persimmon` | _(none — `:root` defaults)_ | no  | Warm cream + persimmon orange (default) |
| `dark`      | `.theme-dark`     | yes | Warm charcoal + persimmon |
| `midnight`  | `.theme-midnight` | yes | Deep navy + lime green accent |
| `plum`      | `.theme-plum`     | yes | Plum velvet + champagne pink |
| `forest`    | `.theme-forest`   | yes | Deep forest green + amber |
| `cobalt`    | `.theme-cobalt`   | no  | Crisp white + cobalt blue |
| `mint`      | `.theme-mint`     | no  | Cool slate + mint |
| `mono`      | `.theme-mono`     | no  | Pure greyscale, zero chroma |

The React glue (in `designs/app.jsx`) keeps a `THEMES` map of `{ label, cls, bg, dark }` and applies the class to the artboard root + body on change. Persist the active theme in `localStorage` + a cookie so SSR can hydrate without a flash. The `dark` flag on each theme lets you set `color-scheme: dark` on the html element for native form controls and scrollbars.

### 5.1 Palette (default `persimmon` theme)
...
Dark mode shifts to `oklch(0.17 / 0.21 / 0.24 / 0.28 60)` for bg/surfaces and lightens accent to `oklch(0.74 0.16 40)`. See `tokens.jsx` for the complete `.theme-dark` block plus the six additional themes (`.theme-midnight`, `.theme-plum`, `.theme-forest`, `.theme-cobalt`, `.theme-mint`, `.theme-mono`).
...
### Theme & density
- Theme selector in user menu (top-right avatar dropdown) — surface the 8 named themes from §5.0 as a list with a small swatch preview. Default is `persimmon`. Persist in `localStorage` + a cookie so SSR can hydrate correctly without flash. Set `color-scheme: dark` on `<html>` for dark themes so native controls match.
- Density toggle (compact / regular / comfy) in the same menu. Same persistence pattern.
...
- `designs/tweaks-panel.jsx` and the `TWEAK_DEFAULTS` block in `app.jsx` — that's a design-time control surface. In production, ship just the **theme selector** (8 named themes — see §5.0) and the **density** control (compact/regular/comfy) as part of the user menu. Drop the "table style" tweak entirely; pick **`lined`** for production (it's the default in the design).
